...
function getSmartSnoozeOptions() {
  ...
  // Morning-aware options
  if (hour < 12) {
    // Morning
    options.push({
      id: "snoozeAfternoon",
      text: "Until 1 PM",
      hours: getHoursUntil(13, 0),
      emoji: "🌞",
    });
  }
  ...
  options.push({
    id: "snoozeMorningAfterNext",
    text: "Two Days",
    hours: getHoursUntil(9, 0, 2),
    emoji: "2️⃣",
  });
  ...
  options.push({
    id: "snoozeWeekend",
    text: "Until Saturday",
    hours: getHoursUntilNextDay(6, 9, 0), // Saturday at 9 AM
    emoji: "🏖️",
  });
...
function getHoursUntil(targetHour, targetMinute, addDays = 0) {
  ...
  // @ts-ignore Date subtraction does work
...
function setupModalHandlers() {
  ...
  document
    .querySelector("#customModal .modal-confirm")
  ...
  document
    .querySelector("#recurringModal .modal-confirm")
    .addEventListener("click", handleRecurringSnooze);
}
  ...
  if (item[alarmName]?.recurringId) {
    ...
    return new Promise((resolve) => {
      ...
      modal.querySelector(".modal-confirm").onclick = () =>
        handleClick("removeAllAndOpen");
      modal.querySelector(".modal-remove-only").onclick = () =>
        handleClick("removeAll");
      modal.querySelector(".modal-remove-single").onclick = () =>
        handleClick("removeSingleAndOpen");
    ...
    return new Promise((resolve) => {
      ...
      modal.querySelector(".modal-confirm").onclick = () =>
        handleClick("removeAndOpen");
      modal.querySelector(".modal-remove-only").onclick = () =>
        handleClick("removeOnly");
      modal.querySelector(".modal-cancel").onclick = () =>
        handleClick("cancel");
    });
  }
...
function listSnoozed() {
  ...
  chrome.storage.local.get(null, (items) => {
    ...
    const regularSnoozes = Object.entries(items).filter(
      ([key, value]) => key.startsWith("snooze-") && !value.recurringId
    );
    const recurringSnoozes = Object.entries(items).filter(
      ([key, value]) => key.startsWith("snooze-") && value.recurringId
    );
    ...
    const addItemToTab = (key, value, container) => {
      ...
      const d = new Date(snoozeTime);
      const localDate =  d.toLocaleDateString();
      const hours = d.getHours().toString().padStart(2, "0");
      const minutes = d.getMinutes().toString().padStart(2, "0");
      const ampm = hours >= 12 ? "PM" : "AM";
      const snoozeTimeStr = `${localDate} ${hours}:${minutes} ${ampm}`;
